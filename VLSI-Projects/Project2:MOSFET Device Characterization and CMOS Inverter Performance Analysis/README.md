...
### 1. Prerequisites
Ensure **ngspice** and a text editor (like **vim**) are installed in your Ubuntu terminal:
```bash
sudo apt update
sudo apt install ngspice vim

```

### 2.Prepare the Environment

* Place all .spice files and the technology.txt (TSMC 0.25μm models) in the same directory.
* If you want to view graphical plots, ensure an X-Server (such as GWSL or VcXsrv) is running on your Windows host.

### 3.View or Edit Circuit Parameters

* To modify transistor dimensions ($W, L$) or change the supply voltage ($V_{DD}$) before running:
```bash
vim filename.spice
```

* Vim basics: Press i to edit, Esc then type :wq to save and exit.
...
* Run the netlist directly with ngspice:
  ...
  * Plots: If the script contains a plot command, a window will pop up showing the VTC or transient waveforms.
  * Data: Numerical results (like Noise Margins or Power measurements) generated by .meas commands will be printed directly to your terminal output.
